...
const UnitsCollection = ({ marketData, walletData }) => {
  const convertToRoman = (num) => {
    const romanNumerals = ['I', 'II', 'III', 'IV', 'V'];
    return romanNumerals[num - 1] || num.toString();
  };

  const unitsByTier = UnitsData.reduce((acc, unit) => {
    if (!acc[unit.tier]) {
      acc[unit.tier] = [];
    }
    ...
    <div className="w-80vw mx-auto mt-8">
      <div className="relative">
        ...
        <div className="absolute inset-x-0 top-0 text-center p-4 text-white font-semibold">
          <p className="bg-gradient-to-b from-sfont1 to-sfont2 text-transparent bg-clip-text text-xl md:text-3xl lg:text-5xl text-center font-bold " style={{ textShadow: '0 0 10px rgba(255, 229, 58, 0.5)', WebkitTextStroke: '0.5px #000000' }}>UNITS</p>
        </div>
      </div>
        ...
        <div key={index} className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
            {unitsByTier[tier].map((unit) => {
              ...
              return (
                <UnitCard key={unit.id} unit={unit} marketData={marketItem} walletData={walletItem} />
              );
            })}
          </div>
        </div>
